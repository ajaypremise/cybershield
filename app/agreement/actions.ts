...
export async function sendAgreementAction(formData: FormData) {
  const data = await parseAgreementAction(formData); const context = await requestContext(); await enforceRateLimit("agreement-send", context.ip, 15, 60 * 60); let failed = false;
  try { await issueSigningRequest(data.agreementId, "SENT"); await adminAudit("AGREEMENT_SENT", data.agreementId, context.ip); } catch { failed = true; }
  if (failed) redirect(`/agreement?agreement=${data.agreementId}&error=${encodeURIComponent("The signing email could not be delivered. The previous link remains unchanged.")}`); redirect(`/agreement?agreement=${data.agreementId}&sent=1`);
}

export async function resendAgreementAction(formData: FormData) {
  const data = await parseAgreementAction(formData); const context = await requestContext(); await enforceRateLimit("agreement-resend", context.ip, 10, 60 * 60); let failed = false;
  try { await issueSigningRequest(data.agreementId, "RESENT"); await adminAudit("AGREEMENT_RESENT", data.agreementId, context.ip); } catch { failed = true; }
  if (failed) redirect(`/agreement?agreement=${data.agreementId}&error=${encodeURIComponent("The replacement signing link could not be delivered.")}`); redirect(`/agreement?agreement=${data.agreementId}&resent=1`);
}
